...
namespace Formats::ELF {
	...
	uint8_t getClassFromArch(Arch arch) {
		switch (arch) {
		case Arch::X86:
		case Arch::ARM32:
			return 0x01;
	...
	uint8_t getDataFromArch(Arch arch) {
		switch (arch) {
		case Arch::X86:
		case Arch::X86_64:
			return 0x01;
	...
	uint16_t getMachineFromArch(Arch arch) {
		switch (arch) {
		case Arch::X86:
			return 0x0003;
		case Arch::X86_64:
	...
	LinkerError link(const LinkerOptions& options, ByteBuffer& bytecode) {
		...
		Arch arch = Arch::X86_64; // TODO: Get arch from input file
		...
		bytecode.addUInt16(EI_CLASS == 0x01 ? 0x0034 : EI_CLASS == 0x02 ? 0x0040 : 0x0000);	// E_EhSize
		bytecode.addUInt16(EI_CLASS == 0x01 ? 0x0020 : EI_CLASS == 0x02 ? 0x0038 : 0x0000);	// E_PhEntSize
		bytecode.addUInt16(programHeaderTableEntryCount);									// E_PhNum
		bytecode.addUInt16(EI_CLASS == 0x01 ? 0x0028 : EI_CLASS == 0x02 ? 0x0040 : 0x0000);	// E_ShEntSize
